#!/usr/bin/env python
"""Run operation sweep: train models for each modular operation, generate figures, print summary."""
...
SKIP_TRAINING = {"addition"}  # Reuse existing baseline
...
def _run_id_for_op(op):
    """Build expected run_id for a given operation."""
...
def print_summary(results_root, operations, logger):
    """Load metrics from all runs and print a comparison table."""
    print("\n" + "=" * 100)
    print("OPERATION SWEEP \u2014 SUMMARY")
    print("=" * 100)
    header = f"{'Operation':>15} | {'Label':>10} | {'Grok Epoch':>10} | {'Train Acc':>9} | {'Test Acc':>8} | {'Gini':>6} | {'W Norm':>8} | Key Freqs"
    print(header)
    print("-" * 100)
    ...
    for op in operations:
        ...
        if not metrics_path.exists():
            dash = "\u2014"
            print(f"{op:>15} | {label:>10} | {'MISSING':>10} | {dash:>9} | {dash:>8} | {dash:>6} | {dash:>8} | {dash}")
            continue
        ...
        print(f"{op:>15} | {label:>10} | {grok_str:>10} | {train_acc:>9.4f} | {test_acc:>8.4f} | {gini:>6.3f} | {w_norm:>8.1f} | {key_freqs}")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Operation sweep for grokking experiments")
    parser.add_argument("--config", type=str, default="configs/sweep_operations.yaml")
    ...
    if args.summary_only:
        print_summary(output_root, operations, logger)
        return
    ...
    for op in operations:
        ...
        print(f"\n{'='*60}")
        logger.info(f"Operation = {op} ({OP_LABELS.get(op, op)}) \u2014 run_id: {rid}")
        ...
        # Skip training for operations we already have
        if op in SKIP_TRAINING:
            logger.info(f"SKIP training: {op} is in SKIP_TRAINING set")
            skipped.append(op)
            continue

        # Train
        logger.info(f"Training {op} for {config['max_epochs']} epochs...")
        t0 = time.time()
        try:
