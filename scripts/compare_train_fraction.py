...
2. Test accuracy vs epoch (log-scale x)
3. Grokking time vs train fraction
4. Gini evolution comparison
5. Weight norm trajectories
6. Train loss comparison
Plus console frequency comparison table.
"""
...
def load_all_runs(results_root, tf_values, p, logger):
...
def get_colormap(tf_values):
    ...
    cmap = plt.get_cmap("plasma", len(tf_values) + 2)
...
def plot_test_accuracy_overlay(runs, colors, p, fig_dir):
    """Figure 1: Test accuracy vs epoch for all train fractions."""
    ...
    for tf in sorted(runs.keys()):
        ...
        n_train = int(p * p * tf)
        ax.plot(epochs, test_acc, color=colors[tf],
                label=f"tf={tf} ({n_train})", linewidth=1.5)
...
def plot_test_accuracy_overlay_log(runs, colors, p, fig_dir):
...
def plot_grokking_time_vs_tf(runs, colors, p, fig_dir):
    ...
    if tf_grok:
        ...
        for t, e in tf_grok:
            n_train = int(p * p * t)
            ax.annotate(f"{e}\n({n_train})", (t, e), textcoords="offset points",
                        xytext=(8, 5), fontsize=9, color=colors[t])

    # Mark non-grokking runs
    for tf in tf_no_grok:
        ax.axvline(x=tf, color=colors[tf], linestyle=":", alpha=0.5)
        n_train = int(p * p * tf)
        ax.annotate(f"tf={tf}\n({n_train})\n(no grok)", (tf, ax.get_ylim()[1] * 0.8),
                    fontsize=9, ha="center", color=colors[tf])

    ax.set_xlabel("Train Fraction", fontsize=12)
    ax.set_ylabel("Grokking Epoch (test acc > 95%)", fontsize=12)
    ax.set_title("Data Efficiency: Grokking Time vs Train Fraction", fontsize=14)
    ax.set_yscale("log")
    ax.grid(True, alpha=0.3, which="both")

    # Add secondary x-axis with number of training examples
    ax2 = ax.twiny()
    ax2.set_xlim(ax.get_xlim())
    tick_tfs = sorted(runs.keys())
    ax2.set_xticks(tick_tfs)
    ax2.set_xticklabels([str(int(p * p * t)) for t in tick_tfs], fontsize=8)
...
def plot_gini_evolution(runs, colors, p, fig_dir):
...
def plot_weight_norm_trajectories(runs, colors, p, fig_dir):
...
def plot_train_loss_overlay(runs, colors, p, fig_dir):
    """Figure 6: Train loss comparison."""
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
...
def print_frequency_table(runs, p, logger):
    """Print key frequency comparison to console."""
    print("\n" + "=" * 90)
    print("KEY FREQUENCY COMPARISON — TRAIN FRACTION SWEEP")
    print("=" * 90)
    print(f"{'Frac':>6} | {'# Train':>7} | {'Gini':>6} | {'Test Acc':>8} | {'Grok Epoch':>10} | Key Frequencies")
    print("-" * 90)
    ...
    for tf in sorted(runs.keys()):
        ...
        n_train = int(p * p * tf)
        print(f"{tf:>6} | {n_train:>7} | {gini:>6.3f} | {test_acc:>8.4f} | {grok_str:>10} | {key_freqs}")
...
def main():
    ...
    parser.add_argument("--p", type=int, default=DEFAULT_P,
                        help="Modular arithmetic prime (default: 113)")
    ...
    # Load all runs
    runs = load_all_runs(results_root, tf_values, p, logger)
    if not runs:
    ...
    plot_test_accuracy_overlay(runs, colors, p, fig_dir)

    logger.info("2/6: Test accuracy overlay (log scale)")
    plot_test_accuracy_overlay_log(runs, colors, p, fig_dir)

    logger.info("3/6: Grokking time vs train fraction")
    plot_grokking_time_vs_tf(runs, colors, p, fig_dir)

    logger.info("4/6: Gini evolution")
    plot_gini_evolution(runs, colors, p, fig_dir)

    logger.info("5/6: Weight norm trajectories")
    plot_weight_norm_trajectories(runs, colors, p, fig_dir)

    logger.info("6/6: Train loss overlay")
    plot_train_loss_overlay(runs, colors, p, fig_dir)

    # Console summary
    print_frequency_table(runs, p, logger)
