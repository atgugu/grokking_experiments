#!/usr/bin/env python3
"""Animated GIF comparing weight decay sweep runs on modular addition.
...
Usage:
    python scripts/animate_wd_sweep.py
    python scripts/animate_wd_sweep.py --frame-step 200 --fps 15
    python scripts/animate_wd_sweep.py --wd-values 0.3 0.5 1.0 2.0
"""
...
DEFAULT_OUTPUT = "results/wd_sweep_comparison/wd_sweep_animation.gif"
...
def load_all_runs(results_root: Path, wd_values: list[float]) -> dict:
    """Load metrics.json for each weight decay run."""
    runs = {}
    for wd in wd_values:
        ...
        with open(metrics_path) as f:
            runs[wd] = json.load(f)
        logger.info("Loaded wd=%s: %s", wd, rid)
    return runs


def load_fourier_data(results_root: Path, wd_values: list[float]) -> dict:
    """Load fourier_snapshots.npz for each weight decay run."""
    fourier = {}
    for wd in wd_values:
        rid = f"p113_d128_h4_mlp512_L1_wd{wd}_s42"
        ...
        if not snap_path.exists():
            ...
            continue
        fourier[wd] = dict(np.load(snap_path))
        logger.info("Loaded Fourier wd=%s (%d snapshots)", wd, len(fourier[wd]["fourier_epochs"]))
    return fourier
...
def get_wd_style(wd_values: list[float], reference_wd: float = 1.0) -> dict:
    """Return {wd: {"color": ..., "linewidth": ..., "zorder": ...}} using tab10."""
    cmap = plt.get_cmap("tab10")
    styles = {}
    for i, wd in enumerate(wd_values):
        is_ref = abs(wd - reference_wd) < 1e-9
        styles[wd] = {
            "color": cmap(i % 10),
...
def preprocess_run_data(runs: dict, fourier_data: dict) -> dict:
    ...
    Returns dict keyed by wd with sub-dicts:
        eval_epochs, test_acc, train_loss, fourier_epochs, gini, key_energy_frac
    """
    processed = {}
    for wd, metrics in runs.items():
        h = metrics["history"]
        entry = {
        ...
        if wd in fourier_data:
            fn = fourier_data[wd]["frequency_norms"]
            entry["key_energy_frac"] = compute_key_energy_fraction(fn)
            entry["key_energy_epochs"] = np.array(
                fourier_data[wd]["fourier_epochs"], dtype=np.float64
            )
        processed[wd] = entry
    return processed
...
def create_wd_sweep_animation(
    ...
    """Build and optionally save the 2x2 weight decay sweep animation."""

    sorted_wds = sorted(run_data.keys())
    ...
    # Create line artists (sorted so reference wd drawn last = on top)
    draw_order = sorted(sorted_wds, key=lambda w: styles[w]["zorder"])

    acc_lines = {}
    loss_lines = {}
    gini_lines = {}
    energy_lines = {}
    for wd in draw_order:
        s = styles[wd]
        kw = dict(color=s["color"], linewidth=s["linewidth"], zorder=s["zorder"], label=s["label"])
        (acc_lines[wd],) = ax_acc.plot([], [], **kw)
        (loss_lines[wd],) = ax_loss.plot([], [], **kw)
        (gini_lines[wd],) = ax_gini.plot([], [], **kw)
        (energy_lines[wd],) = ax_energy.plot([], [], **kw)
    ...
    suptitle = fig.suptitle("Weight Decay Sweep — Epoch 0", fontsize=14, fontweight="bold", y=1.02)
    fig.tight_layout(rect=[0, 0, 1, 0.97])

    def update(frame_idx):
        current_epoch = common_epochs[frame_idx]
        suptitle.set_text(f"Weight Decay Sweep — Epoch {int(current_epoch)}")
        ...
        for wd in sorted_wds:
            ...
            if mask_eval.any():
                ...
                acc_lines[wd].set_data(ep, rd["test_acc"][mask_eval])
                loss_lines[wd].set_data(ep, rd["train_loss"][mask_eval])
            else:
                acc_lines[wd].set_data([], [])
                loss_lines[wd].set_data([], [])

            # Panel C: mask on fourier_epochs
            mask_fourier = rd["fourier_epochs"] <= current_epoch
            if mask_fourier.any():
                gini_lines[wd].set_data(
...
def main():
    parser = argparse.ArgumentParser(
        description="Animate weight decay sweep comparison (2x2 panels)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--wd-values", type=float, nargs="+", default=DEFAULT_WD_VALUES,
        help="Weight decay values to include",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--reference-wd", type=float, default=1.0,
        help="Weight decay value to highlight (thicker line)",
    )
    ...
    wd_values = sorted(args.wd_values)

    logger.info("Loading runs for wd=%s from %s", wd_values, results_root)
    runs = load_all_runs(results_root, wd_values)
    fourier_data = load_fourier_data(results_root, list(runs.keys()))

    if not runs:
        logger.error("No runs found. Check --results-dir and --wd-values.")
        return

    available_wds = sorted(runs.keys())
    logger.info("Available runs: %s", available_wds)

    styles = get_wd_style(available_wds, reference_wd=args.reference_wd)
