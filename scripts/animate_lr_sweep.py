#!/usr/bin/env python3
"""Animated GIF comparing learning rate sweep runs on modular addition.
...
Usage:
    python scripts/animate_lr_sweep.py
    python scripts/animate_lr_sweep.py --frame-step 200 --fps 15
    python scripts/animate_lr_sweep.py --lr-values 1e-4 1e-3 1e-2
"""
...
DEFAULT_OUTPUT = "results/lr_sweep_comparison/lr_sweep_animation.gif"
...
def load_all_runs(results_root: Path, lr_values: list[float]) -> dict:
    """Load metrics.json for each learning rate run."""
    runs = {}
    for lr in lr_values:
        rid = _run_id(lr)
        ...
        with open(metrics_path) as f:
            runs[lr] = json.load(f)
        logger.info("Loaded lr=%.0e: %s", lr, rid)
    return runs


def load_fourier_data(results_root: Path, lr_values: list[float]) -> dict:
    """Load fourier_snapshots.npz for each learning rate run."""
    fourier = {}
    for lr in lr_values:
        rid = _run_id(lr)
        ...
        if not snap_path.exists():
            ...
            continue
        fourier[lr] = dict(np.load(snap_path))
        logger.info("Loaded Fourier lr=%.0e (%d snapshots)", lr, len(fourier[lr]["fourier_epochs"]))
    return fourier
...
def get_lr_style(lr_values: list[float], reference_lr: float = 1e-3) -> dict:
    """Return {lr: {"color": ..., "linewidth": ..., "zorder": ...}} using tab10."""
    cmap = plt.get_cmap("tab10")
    styles = {}
    for i, lr in enumerate(lr_values):
        is_ref = abs(lr - reference_lr) / reference_lr < 1e-6
        styles[lr] = {
            "color": cmap(i % 10),
...
def preprocess_run_data(runs: dict, fourier_data: dict) -> dict:
    ...
    Returns dict keyed by lr with sub-dicts:
        eval_epochs, test_acc, train_loss, fourier_epochs, gini, key_energy_frac
    """
    processed = {}
    for lr, metrics in runs.items():
        h = metrics["history"]
        entry = {
        ...
        if lr in fourier_data:
            fn = fourier_data[lr]["frequency_norms"]
            entry["key_energy_frac"] = compute_key_energy_fraction(fn)
            entry["key_energy_epochs"] = np.array(
                fourier_data[lr]["fourier_epochs"], dtype=np.float64
            )
        processed[lr] = entry
    return processed
...
def create_lr_sweep_animation(
    ...
    """Build and optionally save the 2x2 learning rate sweep animation."""

    sorted_lrs = sorted(run_data.keys())
    ...
    # Create line artists (sorted so reference lr drawn last = on top)
    draw_order = sorted(sorted_lrs, key=lambda l: styles[l]["zorder"])

    acc_lines = {}
    loss_lines = {}
    gini_lines = {}
    energy_lines = {}
    for lr in draw_order:
        s = styles[lr]
        kw = dict(color=s["color"], linewidth=s["linewidth"], zorder=s["zorder"], label=s["label"])
        (acc_lines[lr],) = ax_acc.plot([], [], **kw)
        (loss_lines[lr],) = ax_loss.plot([], [], **kw)
        (gini_lines[lr],) = ax_gini.plot([], [], **kw)
        (energy_lines[lr],) = ax_energy.plot([], [], **kw)
    ...
    suptitle = fig.suptitle("Learning Rate Sweep — Epoch 0", fontsize=14, fontweight="bold", y=1.02)
    fig.tight_layout(rect=[0, 0, 1, 0.97])

    def update(frame_idx):
        current_epoch = common_epochs[frame_idx]
        suptitle.set_text(f"Learning Rate Sweep — Epoch {int(current_epoch)}")
        ...
        for lr in sorted_lrs:
            ...
            if mask_eval.any():
                ...
                acc_lines[lr].set_data(ep, rd["test_acc"][mask_eval])
                loss_lines[lr].set_data(ep, rd["train_loss"][mask_eval])
            else:
                acc_lines[lr].set_data([], [])
                loss_lines[lr].set_data([], [])

            # Panel C: mask on fourier_epochs
            mask_fourier = rd["fourier_epochs"] <= current_epoch
            if mask_fourier.any():
                gini_lines[lr].set_data(
...
def main():
    parser = argparse.ArgumentParser(
        description="Animate learning rate sweep comparison (2x2 panels)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Learning rate values to include",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--reference-lr", type=float, default=1e-3,
        help="Learning rate value to highlight (thicker line)",
    )
    ...
    lr_values = sorted(args.lr_values)

    logger.info("Loading runs for lr=%s from %s", lr_values, results_root)
    runs = load_all_runs(results_root, lr_values)
    fourier_data = load_fourier_data(results_root, list(runs.keys()))

    if not runs:
        logger.error("No runs found. Check --results-dir and --lr-values.")
        return

    available_lrs = sorted(runs.keys())
    logger.info("Available runs: %s", available_lrs)

    styles = get_lr_style(available_lrs, reference_lr=args.reference_lr)
